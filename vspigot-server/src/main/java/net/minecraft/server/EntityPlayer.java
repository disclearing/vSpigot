...
import net.valorhcf.block.BlockPosition;
import net.valorhcf.block.BlockPositionData;
import net.valorhcf.block.ChunkPosition;
...
public class EntityPlayer extends EntityHuman implements ICrafting {
    ...
    private final ConcurrentMap<BlockPosition, MaterialData> fakeBlockMap = new ConcurrentHashMap<>();
    ...
    public ConcurrentMap<BlockPosition, MaterialData> getFakeBlocks() {
        return this.fakeBlockMap;
    }

    public void clearFakeBlocks(final boolean send) {
        if (send) {
            this.setFakeBlocks(Collections.emptyMap(), new ArrayList<>(this.fakeBlockMap.keySet()), true);
        }
        else {
    ...
    public void setFakeBlocks(final Map<BlockPosition, MaterialData> blockMap, final Collection<BlockPosition> replace, final boolean send) {
        if (this.playerConnection != null) {
            final HashMultimap<net.valorhcf.block.ChunkPosition, BlockPositionData> map = HashMultimap.create();
            for (final Map.Entry<BlockPosition, MaterialData> entry : blockMap.entrySet()) {
                ...
                final MaterialData previous = this.fakeBlockMap.put(blockPosition, materialData);
                if (send && previous != materialData) {
                    ...
                    int posX = x - (chunkX << 4);
                    int posZ = z - (chunkZ << 4);
                    map.put(new net.valorhcf.block.ChunkPosition(chunkX, chunkZ), new BlockPositionData(new BlockPosition(posX, y, posZ), materialData));
                }
            }
            for (final BlockPosition blockPosition2 : replace) {
                if (this.fakeBlockMap.remove(blockPosition2) != null) {
                    int x2 = blockPosition2.getX();
                    int y2 = blockPosition2.getY();
                    int z2 = blockPosition2.getZ();
                    int type = CraftMagicNumbers.getId(this.world.getType(blockPosition2.getX(), blockPosition2.getY(), blockPosition2.getZ()));
                    int data = this.world.getData(blockPosition2.getX(), blockPosition2.getY(), blockPosition2.getZ());
                    int chunkX2 = x2 >> 4;
                    int chunkZ2 = z2 >> 4;
                    int posX2 = x2 - (chunkX2 << 4);
                    int posZ2 = z2 - (chunkZ2 << 4);
                    map.put(new net.valorhcf.block.ChunkPosition(chunkX2, chunkZ2), new BlockPositionData(new BlockPosition(posX2, y2, posZ2), new MaterialData(type, (byte) data)));
                }
            }
            if (send) {
                for ( Map.Entry<ChunkPosition, Collection<BlockPositionData>> entry2 : map.asMap().entrySet()) {
                    net.valorhcf.block.ChunkPosition chunkPosition = entry2.getKey();
                    Collection<BlockPositionData> blocks = entry2.getValue();
                    int dirtyCount = blocks.size();
                    ByteArrayDataOutput byteArrayDataOutput = ByteStreams.newDataOutput();
                    short[] dirtData = new short[dirtyCount];
                    int[] blockData = new int[dirtyCount];
                    int i = 0;
                    for (final BlockPositionData blockPositionData : blocks) {
                        final BlockPosition blockPosition3 = blockPositionData.getBlockPosition();
                        final MaterialData materialData2 = blockPositionData.getMaterialData();
                        final int blockId = materialData2.getItemTypeId();
                        int data2 = materialData2.getData();
                        data2 = SpigotDebreakifier.getCorrectedData(blockId, data2);
                        final short dirty = (short)(blockPosition3.getX() << 12 | blockPosition3.getZ() << 8 | blockPosition3.getY());
                        final int blocky = (blockId & 0xFFF) << 4 | (data2 & 0xF);
                        dirtData[i] = dirty;
                        blockData[i] = blocky;
                        byteArrayDataOutput.writeShort(dirty);
                        byteArrayDataOutput.writeShort((short)blocky);
                        ++i;
                    }
                    final PacketPlayOutMultiBlockChange packetPlayOutMultiBlockChange = PacketPlayOutMultiBlockChange.
                            create(new ChunkCoordIntPair(chunkPosition.getX(), chunkPosition.getZ()), byteArrayDataOutput.toByteArray(), dirtyCount, dirtData, blockData);
                    this.playerConnection.sendPacket(packetPlayOutMultiBlockChange);
                }
            }
        }
    }
