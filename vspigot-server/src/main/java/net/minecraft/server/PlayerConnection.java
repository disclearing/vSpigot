...
import net.valorhcf.ValorSpigot;
...
public class PlayerConnection implements PacketPlayInListener {
    ...
    public void a(PacketPlayInFlying packetplayinflying) {
        ...
        if (!this.player.viewingCredits) {
            ...
            if (packetplayinflying.hasPos && delta > 0.0D && this.checkMovement && !this.player.dead) {
                for (MovementHandler handler : ValorSpigot.INSTANCE.getMovementHandlers()) {
                    try {
            ...
            if (this.checkMovement && !this.player.dead) {
                ...
                // Poweruser start
                if (d10 > org.spigotmc.SpigotConfig.movedTooQuicklyThreshold && (!this.minecraftServer.N() || !this.minecraftServer.M().equals(this.player.getName()))) { // CraftBukkit - Added this.checkMovement condition to solve this check being triggered by teleports
                    c.warn(this.player.getName() + " moved too quickly! Distance: " + Math.sqrt(d10));
    ...
    public void a(IChatBaseComponent ichatbasecomponent) {
        ...
        try {
            ...
            c.info(this.player.getName() + " lost connection: I'M NOT TELLING YOU WHY BECAUSE SHIT WOULD BREAK SORRY");
            e.printStackTrace();
            System.out.println(ichatbasecomponent);
        }
    ...
    public void sendPacket(Packet packet) {
        ...
        try {
            ...
            for (PacketHandler handler : ValorSpigot.INSTANCE.getPacketHandlers()) {
                try {
