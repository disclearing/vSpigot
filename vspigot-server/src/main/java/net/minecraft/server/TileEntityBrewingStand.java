...
public class TileEntityBrewingStand extends TileEntity implements IWorldInventory {
    ...
    private void l() {
        if (this.k()) {
            ...
            final ItemStack[] newItems = new ItemStack[this.items.length];
            for (int i = 0; i < this.items.length; ++i) {
                final ItemStack next = this.items[i];
                newItems[i] = ((next == null) ? null : next.cloneItemStack());
            }
            for (int i = 0; i < 3; ++i) {
                if (newItems[i] != null && newItems[i].getItem() == Items.POTION) {
                    final int j = newItems[i].getData();
                    final int k = this.c(j, itemstack);
                    final List list = Items.POTION.c(j);
                    final List list2 = Items.POTION.c(k);
                    if (k > 0 && PotionsConfig.isBrewingDisabled(k)) {
                        continue;
                    }
                    if ((j <= 0 || list != list2) && (list == null || (!list.equals(list2) && list2 != null))) {
                        if (j != k) {
                            newItems[i].setData(k);
                        }
            ...
            if (itemstack.getItem().u()) {
                newItems[3] = new ItemStack(itemstack.getItem().t());
            } else {
                final ItemStack itemStack = newItems[3];
                --itemStack.count;
                if (newItems[3].count <= 0) {
                    newItems[3] = null;
                }
            }
            if (BrewEvent.getHandlerList().getRegisteredListeners().length != 0 && this.getOwner() != null) {
                final org.bukkit.inventory.ItemStack[] results = new org.bukkit.inventory.ItemStack[newItems.length];
                for (int l = 0; l < newItems.length; ++l) {
                    final ItemStack item = newItems[l];
                    results[l] = ((item == null) ? null : CraftItemStack.asBukkitCopy(item));
                }
                final BrewEvent event = new BrewEvent(this.world.getWorld().getBlockAt(this.x, this.y, this.z), (BrewerInventory) this.getOwner().getInventory(), results);
